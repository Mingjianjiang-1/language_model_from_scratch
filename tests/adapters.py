...
from cs336_basics.models.item_layers import FeedForward, RMSNorm, GeLU
from cs336_basics.models.attention import CausalSelfMultiHeadAttention
from cs336_basics.models.transformer import Transformer, TransformerBlock
import gc
...
def run_positionwise_feedforward(
    ...
    ffn.linear1.weight.data = weights["w1.weight"]
    ffn.linear2.weight.data = weights["w2.weight"]
...
def run_scaled_dot_product_attention(
    ...
    if pdrop is not None:
        attention = torch.nn.functional.dropout(attention, pdrop)
    return attention
...
def run_multihead_self_attention(
    ...
    """
    model = CausalSelfMultiHeadAttention(d_model, num_heads, attn_pdrop)
    model.query.weight.data = torch.cat([weights[f"q_heads.{i}.weight"] for i in range(num_heads)], dim=0) 
    model.key.weight.data = torch.cat([weights[f"k_heads.{i}.weight"] for i in range(num_heads)], dim=0) 
    model.value.weight.data = torch.cat([weights[f"v_heads.{i}.weight"] for i in range(num_heads)], dim=0) 
    model.output.weight.data = weights["output_proj.weight"]
    # TODO: Question? 
...
def run_transformer_block(
    ...
    """
    model = TransformerBlock(d_model, num_heads, d_ff, attn_pdrop, residual_pdrop)
    model.attn.query.weight.data = weights["attn.q_proj.weight"]
    model.attn.key.weight.data = weights["attn.k_proj.weight"]
    model.attn.value.weight.data = weights["attn.v_proj.weight"]
    model.attn.output.weight.data = weights["attn.output_proj.weight"]
    model.norm1.weight.data = weights["ln1.weight"]
    model.feedforward.linear1.weight.data = weights["ffn.w1.weight"]
    model.feedforward.linear2.weight.data = weights["ffn.w2.weight"]
    model.norm2.weight.data = weights["ln2.weight"]
...
def run_transformer_lm(
    ...
    """
    model = Transformer(vocab_size, context_length, num_layers, d_model, num_heads, d_ff, attn_pdrop, residual_pdrop, pre_norm=False)
    model.word_embedding.weight.data = weights["token_embeddings.weight"]
    model.position_embedding.weight.data = weights["position_embeddings.weight"]
    model.norm.weight.data = weights["ln_final.weight"]
    # model.layers = torch.nn.ModuleList([TransformerBlock(d_model, num_heads, d_ff, attn_pdrop, residual_pdrop) for _ in range(num_layers)])
    for i in range(num_layers):
        model.layers[i].attn.query.weight.data = weights[f"layers.{i}.attn.q_proj.weight"]
        model.layers[i].attn.key.weight.data = weights[f"layers.{i}.attn.k_proj.weight"]
        model.layers[i].attn.value.weight.data = weights[f"layers.{i}.attn.v_proj.weight"]
        model.layers[i].attn.output.weight.data = weights[f"layers.{i}.attn.output_proj.weight"]
        model.layers[i].norm1.weight.data = weights[f"layers.{i}.ln1.weight"]
        model.layers[i].feedforward.linear1.weight.data = weights[f"layers.{i}.ffn.w1.weight"]
        model.layers[i].feedforward.linear2.weight.data = weights[f"layers.{i}.ffn.w2.weight"]
        model.layers[i].norm2.weight.data = weights[f"layers.{i}.ln2.weight"]
    
    model.output.weight.data = weights["lm_head.weight"]
...
def run_rmsnorm(
    ...
    rmsnorm.weight.data = weights["weight"]
...
def run_get_lr_cosine_schedule(
    ...
    """
    return lr_cosine_schedule(it, max_learning_rate, min_learning_rate, T_w=warmup_iters, T_c=cosine_cycle_iters)   
